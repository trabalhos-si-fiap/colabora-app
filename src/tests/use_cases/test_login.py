...
class TestLoginUseCase(unittest.TestCase):
    ...
    def test_execute_login_success(self, mock_password_manager, mock_user_repository, mock_input):
        # Arrange
        ...
        mock_pm_instance.check_password.assert_called_once_with('password123', mock_user)
        self.assertEqual(result, mock_user)

    @patch('builtins.input', side_effect=['nonexistentuser', 'password123'])
    @patch('src.use_cases.login.UserRepository')
    @patch('src.use_cases.login.PasswordManager')
    def test_execute_login_user_not_found(self, mock_password_manager, mock_user_repository, mock_input):
        # Arrange
        mock_repo_instance = MagicMock()
    ...
    @patch('src.use_cases.login.UserRepository')
    @patch('src.use_cases.login.PasswordManager')
    def test_execute_login_incorrect_password(self, mock_password_manager, mock_user_repository, mock_input):
        # Arrange
        mock_repo_instance = MagicMock()
        mock_user = User('testuser', 'hashed_password', 'salt')
        mock_repo_instance.find_user.return_value = mock_user
        ...
        # Assert
        mock_repo_instance.find_user.assert_called_once_with('testuser')
        mock_pm_instance.check_password.assert_called_once_with('wrongpassword', mock_user)
        self.assertIsNone(result)
