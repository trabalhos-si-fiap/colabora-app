...
class ProjectScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.user_id:
            self.user = self._user_repo.get_by_id_with_all_relations(self.user_id)

        yield Header(show_clock=True)
        with VerticalScroll(classes="bg with-border"):
            yield Static()  # Para funcionar o espaçamento
            yield Static("[b]Projetos Disponíveis[/]", classes="text title pb pt")
            yield Static(
    ...
    def handle_subscription(self, event: Button.Pressed):
        ...
        if "subscribe_btn" in event.button.id:
            ...
            if self.user.is_subscribed_to(project_to_toggle):
                ...
                project_with_habilities = self._project_repo.get_by_id_with_habilities(
                    project_id
                )
                my_projects_container.mount(
                    self._create_project_widget(project_with_habilities, prefix="my")
                )
